...
public:
    ...
    [[nodiscard]] Analysis analyse(const float* samples, int sampleCount) const noexcept
    {
        ...
        if(!primitiveLowered&&sampleCount>=samplesForMs(21.333)&&0.5*hz>=minimumHz_){auto low=exclusiveLowerFamily(x,sampleCount,bestLag);if(low.observable&&low.witnesses8>=3){hz=low.hz;finalLag=low.lag;primitiveLowered=true;exclusiveLower=true;}}
...
private:
    ...
    struct LowerFamily{bool observable=false;double hz=0;int lag=0,witnesses8=0;};
    ...
    LowerFamily exclusiveLowerFamily(const std::array<double,maxAnalysisSamples>&x,int sampleCount,int shortLag)const noexcept{if(shortLag<=0)return{};int centre=2*shortLag,radius=std::max(2,(int)std::ceil(0.05*centre)),lo=std::max(shortLag+2,centre-radius),hi=std::min(sampleCount-40,centre+radius);if(lo>hi)return{};int overlap=sampleCount-hi;if(overlap<40)return{};const double shortMismatch=mismatch(x,sampleCount,overlap,shortLag);double best=std::numeric_limits<double>::infinity();int bestLag=0;for(int lag=lo;lag<=hi;++lag){double m=mismatch(x,sampleCount,overlap,lag);if(m<best){best=m;bestLag=lag;}}if(bestLag<=0)return{};if(best/std::max(1e-12,shortMismatch)>1.20)return{};double lowHz=sampleRate_/bestLag;if(!(lowHz>=minimumHz_))return{};int wits=0,measured=0;for(int k:{1,3,5,7}){double hz=lowHz*k;if(hz>=0.44*sampleRate_)continue;double cp=projectionPower(x,sampleCount,sampleRate_,hz);std::array<double,4>side{projectionPower(x,sampleCount,sampleRate_,hz-0.40*lowHz),projectionPower(x,sampleCount,sampleRate_,hz-0.30*lowHz),projectionPower(x,sampleCount,sampleRate_,hz+0.30*lowHz),projectionPower(x,sampleCount,sampleRate_,hz+0.40*lowHz)};std::sort(side.begin(),side.end());double floor=0.5*(side[1]+side[2]),ratio=cp/std::max(1e-20,floor);if(ratio>=8.0)++wits;++measured;}if(measured<3)return{};return{true,lowHz,bestLag,wits};}
