...
{
public:
    explicit ParkedWorker(Tracker& tracker, int mask = kWorkerMask)
        : tracker_(tracker), mask_(mask)
    {
...
CandidateSet runDualParked(Tracker& tracker,
                          ParkedWorker& halfWorker,
                          ParkedWorker& eighthWorker,
                          Tracker::AnalysisWorkspace& mainWorkspace)
{
    CandidateSet output {};
    const auto halfTicket = halfWorker.submit();
    const auto eighthTicket = eighthWorker.submit();

    runMask(tracker, kMainMask, mainWorkspace, output);

    halfWorker.wait(halfTicket);
    eighthWorker.wait(eighthTicket);

    const auto halfOutput = halfWorker.result();
    const auto eighthOutput = eighthWorker.result();
    output.candidate[1] = halfOutput.candidate[1];
    output.candidate[3] = eighthOutput.candidate[3];
    return output;
}

bool verifyDualParked(Tracker& tracker)
{
    ParkedWorker halfWorker(tracker, kHalf);
    ParkedWorker eighthWorker(tracker, kEighth);
    ...
    for (int iteration = 0; iteration < 300; ++iteration)
    {
        ...
        const auto parallel = runDualParked(
            tracker, halfWorker, eighthWorker, mainWorkspace);

        if (!sameSet(serial, parallel, kSuperMask))
        {
            std::cerr << "PARKED_WORKER_CENSUS=FAIL reason=dual_bit_mismatch"
                      << " iteration=" << iteration << '\n';
            return false;
        }
    }
...
WorkerMetrics measureDualParked(Tracker& tracker)
{
    ParkedWorker halfWorker(tracker, kHalf);
    ParkedWorker eighthWorker(tracker, kEighth);
    Tracker::AnalysisWorkspace mainWorkspace {};
    ...
    for (int iteration = 0; iteration < 30; ++iteration)
        (void) runDualParked(tracker, halfWorker, eighthWorker, mainWorkspace);
...
int main()
{
    ...
    if (!verifyWorker<ParkedWorker>(tracker, "parked"))
        return 4;
    if (!verifyDualParked(tracker))
        return 5;

    std::cout << "PARKED_WORKER_BIT_EQUIVALENCE=PASS repetitions=300\n";
    std::cout << "DUAL_PARKED_WORKER_BIT_EQUIVALENCE=PASS repetitions=300\n";

    const bool parkedFirst = std::getenv("NEUMATON_PARKED_FIRST") != nullptr;

    WorkerMetrics hot;
    WorkerMetrics parked;
    WorkerMetrics dualParked;

    if (parkedFirst)
    {
        dualParked = measureDualParked(tracker);
    ...
    printMetrics("dual_parked", dualParked);
    ...
    const double dualVsSingleP99 =
        100.0 * (1.0 - dualParked.cadenced.percentile(0.99)
                         / std::max(1.0e-9, parked.cadenced.percentile(0.99)));
    const double dualVsSingleMax =
        100.0 * (1.0 - dualParked.cadenced.maximum()
                         / std::max(1.0e-9, parked.cadenced.maximum()));
    const double dualCpuDelta =
        100.0 * (dualParked.cadenceCpuRatio
                         / std::max(1.0e-9, parked.cadenceCpuRatio) - 1.0);

    std::cout << "DUAL_PARKED_COMPARISON"
              << " cadence_p99_reduction_vs_single_pct=" << dualVsSingleP99
              << " cadence_max_reduction_vs_single_pct=" << dualVsSingleMax
              << " process_cpu_ratio_delta_vs_single_pct=" << dualCpuDelta
              << " single_cpu_ratio=" << parked.cadenceCpuRatio
              << " dual_cpu_ratio=" << dualParked.cadenceCpuRatio
              << '\n';
