                          ...
                          std::uint64_t& nonFiniteSamples)
{
    ...
    while (rendered < samplesToRender)
    {
        ...
        for (int sample = 0; sample < blockSize; ++sample)
        {
            ...
            if (!std::isfinite(left)) ++nonFiniteSamples;
            if (!std::isfinite(right)) ++nonFiniteSamples;
            totalOutputEnergy += static_cast<double>(left) * left
                + static_cast<double>(right) * right;
            maximumAbsoluteSample = std::max(maximumAbsoluteSample,
...
int main(int argc, char** argv)
{
    if (argc != 3)
    {
        std::cerr << "usage: v3_audio_renderer <audio.raw> <stats.txt>\n";
        return 2;
    }
    ...
    stats << "guarded_allocations=" << allocations << '\n'
          ...
          << "total_output_energy=" << totalOutputEnergy << '\n'
          << "maximum_absolute_sample=" << maximumAbsoluteSample << '\n'
    ...
    {
        std::cerr << "V3 ledger did not observe valid frames\n";
        return 6;
    }
    if (nonFiniteSamples != 0u)
    {
        std::cerr << "non-finite V3 output sample detected\n";
        return 7;
    }
    if (!(totalOutputEnergy > 1.0e-4))
    {
        std::cerr << "V3 output remained silent\n";
        return 8;
    }
    if (!(maximumAbsoluteSample < 16.0f))
    {
        std::cerr << "V3 output exceeded the bounded sample range\n";
        return 9;
    }
