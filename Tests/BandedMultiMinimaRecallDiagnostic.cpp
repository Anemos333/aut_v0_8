...
void run(double f0,double snr,Kind kind,std::uint32_t seed){
 ...
 for(int perBand:{1,2,3,4,6,8}){
  double best=1e9;int candidates=0;
  for(const auto& band:bands){
   const int lo=std::max(2,int(std::floor(sr/band[1])));
   const int hi=std::min(n-16,int(std::ceil(sr/band[0])));
   std::vector<M> mins;
   if(lo<=hi){
    for(int tau=std::max(lo+1,2);tau<std::min(hi,n-16);++tau){
     float v=ws.difference[size_t(tau)];
     if(v<=ws.difference[size_t(tau-1)]&&v<ws.difference[size_t(tau+1)])mins.push_back({tau,v});
    }
    if(mins.empty()){
      int bt=lo;float bv=ws.difference[size_t(lo)];
      for(int tau=lo+1;tau<=hi;++tau)if(ws.difference[size_t(tau)]<bv){bv=ws.difference[size_t(tau)];bt=tau;}
      mins.push_back({bt,bv});
    }
    std::sort(mins.begin(),mins.end(),[](auto&a,auto&b){return a.v<b.v;});
    int take=std::min(perBand,int(mins.size()));
    for(int i=0;i<take;++i){
      const double parent=sr/double(mins[size_t(i)].tau);
      for(int d=1;d<=4;++d){
       double hz=parent/double(d);if(hz<55||hz>1600)continue;
       ++candidates;best=std::min(best,std::abs(cents(hz,f0)));
      }
    }
   }
  }
  std::cout<<std::fixed<<std::setprecision(6)
           <<"BANDED_MULTI_RECALL kind="<<name(kind)<<" hz="<<f0<<" snr="<<snr<<" seed="<<seed
           <<" per_band="<<perBand<<" candidates="<<candidates<<" best_abs_cents="<<best
