...
18. **Legacy intelligence returns only as evidence.** Air/breath, harmonicity, event/onset, formant stability, second-harmonic dominance, competing-family/polyphony and spectral-reliability logic may be recovered from `main` only in analysis-only code. `VoiceEvidenceAnalyzer` must accept the input buffer read-only, must not render/delay/mix audio, and must not write `Amount` or any confidence-dependent correction-authority scalar. Its output may condition detector sensitivity, target identity/hysteresis, vibrato admission, transient handling, breath conditioning and LPC/formant supervision inside the one clean path.
